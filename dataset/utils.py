...
def pre_caption(caption, max_words, lowercase=True):
    """Clean a DGM4 caption.

    ``lowercase`` should be ``True`` for uncased models (e.g.
    ``bert-base-uncased``) and ``False`` for case-sensitive models
    (e.g. ``microsoft/deberta-v3-base``). The set of characters stripped /
    replaced is the same in both cases, which keeps word boundaries — and
    therefore the ``fake_text_pos`` indices that are precomputed against
    those word boundaries — aligned regardless of case.
    """
    text = caption.lower() if lowercase else caption
    text = re.sub(r"([,.'!?\"()*#:;~])", '', text)
